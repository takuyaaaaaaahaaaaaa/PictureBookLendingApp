...
原則：**コード変更のPR（feat/fix/refactor等）は、`gh pr create` の前に必ずレビューを通す。**

順序は **実装 → `/simplify` → 独立レビュー → PR作成**。
ただし工程は変更の規模に見合わせる（下表）。かけた手間は必ず釣り合いを確認する。

| 変更の規模 | `/simplify` | 独立レビュー |
| --- | --- | --- |
| ドキュメント・typo | 不要 | 不要 |
| 1ファイルに収まる小さな修正 | 不要（その場で見直す） | `/code-review`（レベル低〜中） |
| 複数ファイル・層をまたぐ | 実施（観点を絞る） | 実施 |

* **先に `/simplify` を回す**。重複・冗長さを畳んでからレビューに掛けることで、
  レビュー対象が最終形になり、簡素化で消えるコードへの指摘に手間をかけずに済む。
  `/simplify` は品質観点のみでバグを探さないため、**レビューの代わりにはならない**
* `/simplify` は4観点を並列エージェントで回すため**コストが大きい**。
  変更の性質に合う観点だけを選ぶ（例：重複が主題ならReuse・Simplificationの2つ、
  設計判断を伴うならAltitudeを加える）。全観点を機械的に回さない
* 実装を行ったのと同じセッション（同じコンテキスト）で自己レビューして済ませない。
  実装者バイアスがかかり、見落としが残りやすいため、**セッションを引き継がない**
  独立したレビューを挟む
* 具体的には `code-reviewer` agent（Agent tool）または `/code-review` を、
  実装の会話とは別コンテキストで実行し、指摘を確認・反映してから PR を作成する
* 指摘への対応要否は判断してよいが、対応しない指摘は理由をPR説明やコメントに残す
* 軽微なtypo修正やドキュメントのみの変更等、レビューの必要性が薄い場合はこの限りではない

### マージ方針

原則：**コード変更のPR（feat/fix/refactor等）は、作成後マージせずオーナーのレビューとGOを待つ。**

* **実機確認＋レビューしてからマージ**：UX・アニメーション・ハプティクス・レイアウト等、
  使用感が本質の変更。調整（表示時間・文言等）はマージ前にPR上で行う
* **レビューしてからマージ**：上記以外のコード変更すべて（テストが通っていても待つ）
* **即マージしてよいのはドキュメントのみ**（docs/・CLAUDE.md等）。
  ただし設計の相談中は、ドキュメントであってもPR作成・マージを待つ
