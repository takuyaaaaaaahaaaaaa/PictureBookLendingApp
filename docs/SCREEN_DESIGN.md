...
### 絵本一覧の表示形式（BookDisplayMode）

絵本一覧はツールバーのメニューから3つの表示形式を切り替えられる。

| 表示形式 | 内容 | 主な用途 |
| --- | --- | --- |
| リスト表示 | サムネイル＋タイトル＋著者の行形式 | 管理作業（設定画面の絵本管理） |
| グリッド表示 | 表紙主体の適応的グリッド | 一覧性重視の閲覧 |
| 棚表示 | 木の本棚メタファーの表示（下記） | 貸出タブでの「絵本コーナー」体験 |

#### 棚表示（木の本棚デザイン）

保育園の絵本コーナーの本棚をメタファーにした表示形式。
「アプリの枠（ナビゲーションバー・検索・タブバー等）は Liquid Glass / HIG 準拠のまま、
**一覧コンテンツエリアだけ**を木の世界にする」ことで、既存のデザイン方針と共存させる。

* **構造**: 五十音セクション＝棚のまとまり。絵本はグリッド表示と同等の大きさのセルを画面幅に応じて折り返して並べ、折り返してできた各行の下に棚板を敷く（本が多いかなグループは棚が複数段になる）。横スクロールは使わず、縦スクロールだけで全冊を見られるようにする（貸出端末としての「探しやすさ」を最優先し、その上で本棚のメタファーを保つ）
* **かなラベル**: 独立した棚札を木地の上に浮かせず、**1つ上の棚板の下にぶら下げる**（先頭グループ用に最上段へ横木を1本渡す）。「棚板の下にぶら下がっている札＝その下に並ぶ絵本の見出し」という一貫した文法で、上から下への読み順と整合させる。ラベルは木の札ではなく**クリーム色の紙ラベル×濃茶文字**とし、木地との同化を避ける
* **配色**:
  * ライトモード: 明るいクリーム〜薄茶の木地（保育園の家具にある明るい木のイメージ）に濃茶の文字
  * ダークモード: 焦げ茶の木地にクリームの文字
  * 視認性の原則: **文字は木目の上に直接乗せない**か、乗せる場合は明るい木地×濃茶文字で十分なコントラストを確保する
* **描画**: 木目はテクスチャ画像を使わず Gradient で軽量に描画する（500冊規模のスクロール性能を担保）
* **状態表示・操作**: BookStatusView・LoanActionContainerButton 等の既存コンポーネントをそのまま使用する（棚表示は見た目のスキンであり、操作体系は変えない）
